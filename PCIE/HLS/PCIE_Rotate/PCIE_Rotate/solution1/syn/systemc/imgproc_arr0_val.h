...
#ifndef _imgproc_arr0_val_H_
#define _imgproc_arr0_val_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "imgproc_arr0_val_memcore.h"

SC_MODULE(imgproc_arr0_val) {
...
protected:
    ...
    // buffer instances
    imgproc_arr0_val_memcore* buffer[BufferCount];
...
public:
    ~imgproc_arr0_val();
    SC_CTOR(imgproc_arr0_val) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "imgproc_arr0_val_memcore_U_";
            name += szTmp;

            buffer[i] = new imgproc_arr0_val_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "imgproc_arr0_val.trace");
            // ports
